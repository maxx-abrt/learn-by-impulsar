...
def keep_alive():
    ...
    while True:
        ...
        time.sleep(300)  # Ping toutes les 10 minutes
...
subdomains = {
    "Anatomie": ["Système osseux", "Système circulatoire", "Système nerveux", "Système urinaire", "Système respiratoire", "Peau"],
...
subsystems = {
    "Système osseux": ["Crâne", "Colonne vertébrale", "Membres supérieurs", "Membres inférieurs", "Système global"],
...
def start_custom_quiz():
    ...
    if current_index < len(questions):
        ...
        user_info = get_user_data(created_by)
        ...
        with st.container(border=True):
            ...
            elif question['type'] == "Réponse courte":
                user_answer = st.text_input(
                    "Votre réponse:",
                    key=f"reponse_courte_{current_index}"
                )
            else:
                st.error(f"Type de question inconnu: {question['type']}")
                user_answer = None  # Ou vous pouvez assigner une valeur par défaut
        
        # Assurer que user_answer est défini avant l'assignation
        if user_answer is not None:
            st.session_state.user_answers[current_index] = user_answer
        else:
            st.session_state.user_answers[current_index] = "Non répondu"
        
        # Détermination du label du bouton
        if current_index == len(questions) - 1:
            button_label = "Finir le QCM"
        else:
            button_label = "Question suivante"
        
        # Gestion du bouton pour passer à la question suivante ou finir le quiz
        ...
        st.warning("Toutes les questions ont été traitées.")
...
def get_recueil_questions_from_db(domaine, ue):
    db = get_db_connection()
    recueil = recueils_questions_collection.find_one({"domaine": domaine, "ue": ue})


    
    if recueil and 'questions' in recueil:
        return recueil['questions']
    else:
        return []
    
if 'recueil_started' not in st.session_state:
    st.session_state.recueil_started = False
if 'current_question_index' not in st.session_state:
    st.session_state.current_question_index = 0
if 'recueil_questions_data' not in st.session_state:
    st.session_state.recueil_questions_data = []

def get_recueil_questions(domaine, ue):
    recueil = recueils_questions_collection.find_one({"domaine": domaine, "ue": ue})
    return recueil['questions'] if recueil else []

def display_recueil_questions_page():
    st.title("Recueil de Questions")
    main_container = st.empty()
    
    if not st.session_state.get('recueil_started', False):
        with main_container.container():
            selected_domaine = st.selectbox("Choisissez un domaine d'étude", ["Infirmier", "Médecine"], key="recueil_domaine_selector")
            selected_ue = st.selectbox("Choisissez l'unité d'enseignement", ["2.1", "2.2"], key="recueil_ue_selector")
            random_order = st.checkbox("Lancer les questions en ordre aléatoire", key="random_order_checkbox")

            recueil_questions = get_recueil_questions(selected_domaine, selected_ue)
            num_questions = len(recueil_questions)
            st.write(f"Nombre de questions disponibles dans le recueil sélectionné : :blue-background[{num_questions}]")
            
            if not recueil_questions:
                st.warning("Aucune question trouvée pour cette sélection.")
                return
            

            if random_order:
                random.shuffle(recueil_questions)
            
            st.session_state.recueil_questions_data = recueil_questions
            
            if st.button(":green[Lancer le recueil]", key="lancer_recueil_button"):
                if not recueil_questions:
                    st.error("Aucune question à lancer.")
                    return
                st.session_state.recueil_started = True
                st.session_state.current_question_index = 0
                st.rerun()
    else:
        with main_container.container():
...
def display_current_question():
    current_index = st.session_state.current_question_index
    recueil_questions = st.session_state.recueil_questions_data
    
    if not isinstance(recueil_questions, list):
        st.error("Les questions du recueil ne sont pas au format attendu.")
        return
    
    if current_index < len(recueil_questions):
        question = recueil_questions[current_index]
        question_id = question.get('_id')
        recueil_id = st.session_state.selected_set_id
        
        question_container = st.empty()
        with question_container.container(border=True):
            progress = (st.session_state.current_question_index + 1) / len(st.session_state.recueil_questions_data)
            st.progress(progress)
            st.subheader(f"Question {current_index + 1}/{len(recueil_questions)}")
            question_text = question.get('question', 'Texte non disponible')
            st.info(question_text)
            
            if st.button(":green[Voir la réponse]", key=f"voir_reponse_{current_index}"):
                answer_text = question.get('réponse', 'Réponse non disponible')
                st.success(answer_text)
            


            button_label = ":red[Terminer le recueil]" if current_index == len(recueil_questions) - 1 else ":orange[Question suivante]"
            
            if st.button(button_label, key=f"next_button_{current_index}"):
                if current_index < len(recueil_questions) - 1:
                    st.session_state.current_question_index += 1
                    st.rerun()
                else:
                    del st.session_state['current_question_index']
                    del st.session_state['recueil_started']
                    del st.session_state['recueil_questions_data']
                    st.success("Recueil terminé.")
                    st.rerun()
...
def display_important_questions(username):
    important_questions = list(important_questions_collection.find({"username": username}))
    
    if not important_questions:
        st.warning("Aucune question importante trouvée.")
        return
    
    for item in important_questions:
        recueil = recueils_questions_collection.find_one({"_id": item['recueil_id']})
        if recueil and 'questions' in recueil:
            question = recueil['questions'][item['question_index']]
            st.subheader(question.get('question', 'Texte non disponible'))
            if st.button(":green[Voir la réponse]", key=f"voir_reponse_importante_{item['question_index']}"):
                answer_text = question.get('réponse', 'Réponse non disponible')
                st.success(answer_text)
    
    
    
if 'questions_fields' not in st.session_state:
    st.session_state.questions_fields = []

def add_question_field():
    st.session_state.questions_fields.append({"question": "", "réponse": ""})

def remove_question_field():
    if st.session_state.questions_fields:
        st.session_state.questions_fields.pop()

def display_creation_recueils_page():
    st.title("Création de Recueils")
    
    # Sélection des critères
    selected_domaine = st.selectbox("Choisissez un domaine d'étude", ["Infirmier", "Médecine"])
    selected_ue = st.selectbox("Choisissez l'unité d'enseignement", ["2.1", "2.2"])  # Exemple d'UE

    col1, col2 = st.columns(2)
    with col1:
        st.button(":green[Ajouter une question]", on_click=add_question_field)
    with col2:
        st.button(":red[Retirer une question]", on_click=remove_question_field)

    for i, field in enumerate(st.session_state.questions_fields):
        with st.container(border=True):
            field["question"] = st.text_input(f"Question {i+1}", value=field["question"], key=f"q_{i}")
            field["réponse"] = st.text_input(f"Réponse {i+1}", value=field["réponse"], key=f"r_{i}")

    if st.button(":blue[Soumettre le recueil]"):
        if not st.session_state.questions_fields:
            st.error("Veuillez ajouter au moins une question avant de soumettre.")
            return

        # Soumettre le recueil en utilisant les données dans questions_fields
        new_recueil = {
            "domaine": selected_domaine,
            "ue": selected_ue,
            "questions": [q for q in st.session_state.questions_fields if q['question'] and q['réponse']]
        }

        # Insérer le recueil dans la base de données
        recueils_questions_collection.insert_one(new_recueil)
        st.success("Recueil soumis avec succès!")

        # Réinitialiser les champs après soumission
        del st.session_state['questions_fields']
...
def create_sidebar():
    with st.sidebar:
        ...
        if st.sidebar.button("📖 Recueil de Questions", key="recueil_questions_button"):
            st.session_state.page = "Recueil de Questions"
            collapse_sidebar()
                    
        if st.sidebar.button("✏️ Créer un Recueil", key="creer_recueil"):
            st.session_state.page = "Création de Recueils"
            collapse_sidebar()
        ...
        if st.sidebar.button("📚 Qcm", key="QCM Personnalisé"):
...
def normalize_answer(answer):
    ...
    answer = ''.join(e for e in answer if e.isalnum() or e.isspace())
    return answer

def is_correct(user_answer, correct_answer, tolerance=1):
    user_answer = normalize_answer(user_answer)
    correct_answer = normalize_answer(correct_answer)

    # Use difflib to check similarity between answers
...
def main():
    ...
    if choice == "Recueil de Questions":
        display_recueil_questions_page()
        
    elif choice == "Création de Recueils":
        display_creation_recueils_page()
